...
(function (i) {
  ...
  i.jqx.observableArray = function (e, J) {
    ...
    I.toJSON = function (X, O) {
      ...
      function M(Z) {
        return (
          '"' +
          Z.replace(T, function (aa) {
            var ab = W[aa];
            return typeof ab === "string"
              ? ab
              : "\\u" + ("0000" + aa.charCodeAt(0).toString(16)).slice(-4);
          }) +
          '"'
        );
      }
      ...
      function R(aa, Z) {
        ...
        if (
          /(number|float|int|integer|string|boolean)/.test(ab) ||
          (!ac && ab === "object")
        ) {
          return V(ac, ab);
        } else {
  ...
  i.jqx.dataAdapter.prototype = {
    ...
    dataBind: function (R, aa) {
      ...
      this.cachedrecords =
        K.cachedrecords != undefined ? K.cachedrecords : new Array();
      ...
      if (Q === "xlsx" && W.url) {
        ...
        if (window.jqxDataSource) {
          var ai = new window.jqxDataSource({
            dataSource: W.url,
            async: false,
            dataFields: W.datafields,
          });
          ai.notify(function () {
            W.url = null;
              ...
              if (typeof ap === "string") {
                ...
                i.each(this, function (ay, aD) {
                    ...
                    for (var ax = 0; ax < ad; ax++) {
                      ...
                      if (aw != undefined && aw.name == ay) {
                        ...
                        if (
                          ...
                          if (ak[aC] != undefined) {
                            ao +=
                              ak[aC].toString().length +
                              ak[aC].toString().substr(0, 1);
                          }
                        }
                      }
                    }
                  ...
                  if (av) {
                    ...
                    if (an[ay].subscribe) {
                      var aA = am;
                      ah.koSubscriptions[ah.koSubscriptions.length] = an[
                        ay
    ...
    buildHierarchy: function (K) {
      ...
      if (e.hierarchy.root) {
        if (e.dataType == "xml") {
          var P = this.getRecordsHierarchy(
            "uid",
            "parentuid",
            "records",
            null,
      ...
      if (e.hierarchy.keyDataField && e.hierarchy.parentDataField) {
        var P = this.getRecordsHierarchy(
          e.hierarchy.keyDataField.name,
          e.hierarchy.parentDataField.name,
          "records",
          null,
          K
        );
    ...
    getRecordsHierarchy: function (L, J, aa, U, H) {
      ...
      var O = ab.length;
      var P =
        this._source && this._source.hierarchy
    ...
    loadjson: function (af, ag, S) {
      ...
      if (
        ...
        throw new Error(
          "jqxDataAdapter: JSON Parse error! Invalid JSON. Please, check your JSON or your jqxDataAdapter initialization!"
        );
        return;
      }
      if (af.length == 0) {
      ...
      if (V.hierarchy && V.hierarchy.root) {
        for (var aa = Y; aa < J; aa++) {
          ...
          if (i(I).parent().length > 0) {
            var ai = this.getid(
              V.id,
              i(I).parents(V.hierarchy.record + ":first")
            );
            N.parentuid = ai;
          } else {
            N.parentuid = null;
    ...
    loadics: function (N, H) {
      ...
      var P = function (V) {
        ...
        return new Date(
          Date.UTC(U[1], U[2] - 1, U[3], U[5] || 0, U[6] || 0, U[7] || 0)
        );
      };
      for (var M = 0; M < K; M++) {
        var O = R[M];
  ...
  i.extend(i.jqx.dataFormat, {
    ...
    getMonthIndex: function (N, M, I) {
      ...
      if (!K) {
        N._upperMonths = K = [
          this.toUpperArray(e.names),
          this.toUpperArray(e.namesAbbr),
        ];
        N._upperMonthsGen = L = [
          this.toUpperArray(H.names),
      ...
      if (i.type(ak) === "string") {
        ...
        if (Y) {
          ...
          for (var aq = 0; aq < L.length; aq++) {
            ...
            var ad =
              ag.parsedate(O, "d", al) ||
              ag.parsedate(O, "dd", al) ||
              ag.parsedate(O, "ddd", al) ||
              ag.parsedate(O, "dddd", al);
            if (ad) {
              if (O.length <= 2) {
                R = aq;
                H.push(ad.getDate());
          ...
          for (var aq = 0; aq < L.length; aq++) {
            ...
            var U =
              ag.parsedate(O, "M", al) ||
              ag.parsedate(O, "MM", al) ||
              ag.parsedate(O, "MMM", al) ||
              ag.parsedate(O, "MMMM", al);
            if (U) {
              if (R != undefined && R == aq) {
                continue;
              }
        ...
        if (I) {
          ...
          var K =
            ag.parsedate(I, "h:mm tt", al) ||
            ag.parsedate(I, "h:mm:ss tt", al) ||
            ag.parsedate(I, "HH:mm:ss.fff", al) ||
            ag.parsedate(I, "HH:mm:ss.ff", al) ||
            ag.parsedate(I, "HH:mm:ss.tttt", al) ||
            ag.parsedate(I, "HH:mm:ss", al) ||
            ag.parsedate(I, "HH:mm", al) ||
            ag.parsedate(I, "HH", al);
          var X = 0,
            J = 0,
            aa = 0,
            ab = 0;
        ...
        if (H) {
          ...
          switch (aa) {
            ...
            case "t":
              ...
              if (
                !U &&
                (!T.AM || (H !== T.AM[0] && H !== T.AM[1] && H !== T.AM[2]))
              ) {
                return null;
              }
              break;
    ...
    formatdate: function (Z, ad, U) {
      ...
      for (;;) {
        ...
        switch (X) {
          ...
          case "tt":
            Y =
              Z.getHours() < 12 ? (U.AM ? U.AM[0] : " ") : U.PM ? U.PM[0] : " ";
    ...
    ajax: function (M, J) {
      ...
      if (!Q.hasContent) {
        ...
        if (Q.cache === false) {
          ...
          Q.url =
            ac + (ac === Q.url ? (k.test(Q.url) ? "&" : "?") + "_=" + H : "");
        }
      }
      if (
        (Q.data && Q.hasContent && Q.contentType !== false) ||
        J.contentType
      ) {
  ...
  i.jqx.data.ajaxPrefilter("json jsonp", function (Q, L, P) {
    var O,
      ...
      K =
        ...
        !M &&
        typeof J === "string" &&
        !(Q.contentType || "").indexOf("application/x-www-form-urlencoded") &&
        D.test(J);
    if (Q.dataTypes[0] === "jsonp" || M || K) {
      O = Q.jsonpCallback = i.isFunction(Q.jsonpCallback)
        ? Q.jsonpCallback()
        : Q.jsonpCallback;
      e = window[O];
      if (M) {
  ...
  i.jqx.filter = function () {
    ...
    this.createfilter = function (Y, V, X, W, U, Z) {
      ...
      throw new Error(
        "jqxGrid: There is no such filter type. The available filter types are: 'numericfilter', 'stringfilter', 'datefilter' and 'booleanfilter'"
      );
      return null;
    };
      ...
      this.evaluate = function (ag) {
        ...
        var am = function (ap, ao) {
          switch (al) {
            ...
            case "ENDS_WITH":
              return i.jqx.string.endsWithIgnoreCase(
                ap.toString(),
                ao.toString()
              );
            case "ENDS_WITH_CASE_SENSITIVE":
            ...
            case "CONTAINS":
              return i.jqx.string.containsIgnoreCase(
                ap.toString(),
                ao.toString()
              );
            case "CONTAINS_CASE_SENSITIVE":
          ...
          if (W != undefined && ab != undefined) {
            if (
              ...
              if (an && an.toString().indexOf(":") == -1) {
                var aj = i.jqx.dataFormat.tryparsedate(
                  an.toString() + ":00",
                  ab
                );
                if (aj != null) {
