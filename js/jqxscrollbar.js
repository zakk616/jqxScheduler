...
(function (a) {
  ...
  a.extend(a.jqx._jqxScrollBar.prototype, {
    ...
    render: function () {
      ...
      if (
        a.jqx.utilities &&
        a.jqx.utilities.scrollBarButtonsVisibility == "hidden"
      ) {
    ...
    _addHandlers: function () {
      ...
      if (j.isTouchDevice || e) {
        this.addHandler(
          ...
          function (k) {
            var l = j.vertical
              ? j.toThemeProperty("jqx-scrollbar-thumb-state-pressed")
              : j.toThemeProperty(
          ...
          switch (l.type) {
            ...
            case "mouseup":
              if (!j.btnDownInstance.base.disabled && j.buttonDownCapture) {
                ...
                j.btnDown.removeClass(
                  j.toThemeProperty("jqx-scrollbar-button-state-pressed")
                );
                j.btnDown.removeClass(
                  j.toThemeProperty("jqx-fill-state-pressed")
                );
      ...
      this.addHandler(this.areaUp, h, function (l) {
        if (!j.disabled) {
          ...
          var n = true;
          var m = j.btnThumb.offset();
          if (!j.vertical) {
            if (j.areaClickOffset.x >= m.left) {
              n = false;
            }
          } else {
            if (j.areaClickOffset.y >= m.top) {
              n = false;
            }
          }
          if (n) {
      ...
      this.addHandler(this.areaUp, "mousedown", function (k) {
        if (!j.disabled) {
          ...
          j.areaClickOffset = { x: k.pageX, y: k.pageY };
          return false;
        }
      });
      ...
      if (!this.isTouchDevice) {
        ...
        this.addHandler(
          a(document),
          "mousemove." + this.element.id,
          this.mousemoveFunc
        );
        this.addHandler(
          a(document),
          "mouseleave." + this.element.id,
          function (k) {
            if (!j.disabled) {
        ...
        if (!j.disabled) {
          this.addHandler(this.btnUp, "mouseenter mouseleave", function (k) {
            if (k.type === "mouseenter") {
              if (
                !j.disabled &&
                !j.btnUpInstance.base.disabled &&
                j.touchMode != true
              ) {
                j.btnUp.addClass(
                  j.toThemeProperty("jqx-scrollbar-button-state-hover")
                );
          ...
          this.addHandler(this.btnThumb, "mouseenter mouseleave", function (k) {
            if (k.type === "mouseenter") {
              ...
              if (!j.disabled && j.touchMode != true) {
                ...
                j.btnThumb.removeClass(
                  j.toThemeProperty("jqx-fill-state-hover")
                );
              }
            }
          });
          this.addHandler(this.btnDown, "mouseenter mouseleave", function (k) {
            if (k.type === "mouseenter") {
              if (
                !j.disabled &&
                !j.btnDownInstance.base.disabled &&
                j.touchMode != true
              ) {
                j.btnDown.addClass(
                  j.toThemeProperty("jqx-scrollbar-button-state-hover")
    ...
    _addArrowClasses: function (c, b) {
      ...
      if (this.vertical) {
        ...
        if (b == "down" || b == undefined) {
          this.arrowDown.addClass(
            this.toThemeProperty("jqx-icon-arrow-down" + c)
          );
        }
      } else {
        if (b == "up" || b == undefined) {
          this.arrowUp.addClass(
            this.toThemeProperty("jqx-icon-arrow-left" + c)
          );
        }
        if (b == "down" || b == undefined) {
          this.arrowDown.addClass(
            this.toThemeProperty("jqx-icon-arrow-right" + c)
          );
        }
      }
    },
    _removeArrowClasses: function (c, b) {
      ...
      if (this.vertical) {
        if (b == "up" || b == undefined) {
          this.arrowUp.removeClass(
            this.toThemeProperty("jqx-icon-arrow-up" + c)
          );
        }
        if (b == "down" || b == undefined) {
          this.arrowDown.removeClass(
            this.toThemeProperty("jqx-icon-arrow-down" + c)
          );
        }
      } else {
    ...
    setTheme: function () {
      ...
      var d =
        this.toThemeProperty("jqx-scrollbar") +
        " " +
        this.toThemeProperty("jqx-widget") +
        " " +
        this.toThemeProperty("jqx-widget-content");
      this.host.addClass(d);
      if (this.isTouchDevice) {
        this.host.addClass(this.toThemeProperty("jqx-scrollbar-mobile"));
      }
      m[0].className = this.toThemeProperty(
        "jqx-scrollbar-button-state-normal"
      );
      o[0].className = this.toThemeProperty(
        "jqx-scrollbar-button-state-normal"
      );
      var q = "";
      if (this.vertical) {
      ...
      if (o.css("display") === "none") {
        ...
        p.addClass(
          this.toThemeProperty("jqx-scrollbar-thumb-state-normal-touch")
        );
      }
    ...
    isScrolling: function () {
      if (
        this.thumbCapture == undefined ||
        this.buttonDownCapture == undefined ||
        this.buttonUpCapture == undefined ||
        this.areaDownCapture == undefined ||
        this.areaUpCapture == undefined
      ) {
        return false;
      }
      return (
        this.thumbCapture ||
        this.buttonDownCapture ||
        this.buttonUpCapture ||
        this.areaDownCapture ||
        this.areaUpCapture
      );
    },
    track: function () {
    ...
    handlemousedown: function (e) {
      if (this.thumbCapture == undefined || this.thumbCapture == false) {
        ...
        if (c != null) {
          ...
          if (this.vertical) {
            c.addClass(
              this.toThemeProperty("jqx-scrollbar-thumb-state-pressed")
            );
          } else {
            c.addClass(
    ...
    handlemouseleave: function (e) {
      ...
      if (this.buttonDownCapture || this.buttonUpCapture) {
        b.removeClass(
          this.toThemeProperty("jqx-scrollbar-button-state-pressed")
        );
        d.removeClass(
          this.toThemeProperty("jqx-scrollbar-button-state-pressed")
        );
      ...
      var f = this.vertical
        ? this.toThemeProperty("jqx-scrollbar-thumb-state-pressed")
        : this.toThemeProperty("jqx-scrollbar-thumb-state-pressed-horizontal");
      c.removeClass(f);
    ...
    handlemousemove: function (b) {
      ...
      try {
        ...
        if (!this._btnAndThumbSize) {
          f = this.vertical
            ? i.height() + e.height() + k.height()
            : i.width() + e.width() + k.width();
        }
        ...
        if (this.thumbStep == "auto") {
          ...
          l *= g;
          if (
            Math.abs(this.dragStartValue + l - this.value) >=
            parseInt(this.thumbStep)
          ) {
        ...
        if (a.jqx.scrollAnimation) {
          ...
          function f() {
            ...
            if (d.amplitude) {
              ...
              if (o > 0.5 || o < -0.5) {
                var l =
                  (d.max - d.min) / (d.scrollBarSize - d._btnAndThumbSize);
                var n = l * (d.target + o);
                var m = n;
                if (d.rtl && !d.vertical) {
                  m = -n;
    ...
    _positionelements: function () {
      ...
      if (!this._btnAndThumbSize) {
        var i = this.vertical
          ? 2 * this.btnSize + o.outerHeight()
          : 2 * this.btnSize + o.outerWidth();
        i = Math.round(i);
      }
      var k = ((m - i) / (this.max - this.min)) * (this.value - this.min);
      if (this.rtl && !this.vertical) {
        k =
          ((m - i) / (this.max - this.min)) *
          (this.max - this.value - this.min);
      }
      k = Math.round(k);
      if (k < 0) {
        k = 0;
    ...
    _arrange: function () {
      ...
      m.btnSize = b;
      var e = m.vertical
        ? 2 * s + (2 + parseInt(u[0].style.height))
        : 2 * s + (2 + parseInt(u[0].style.width));
      e = Math.round(e);
